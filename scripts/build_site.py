...
function isFiltering() {{
  return searchQuery !== '' || activeCat !== 'all' || activeSt !== 'all';
}}
function applyFilter() {{
  const filtering = isFiltering();
  document.getElementById('normalView').hidden = filtering;
  document.getElementById('searchResults').hidden = !filtering;
  // 검색/유형/상태 필터 결과는 연도와 무관하게 전체에서 찾으므로, 필터가 걸려
  // 있는 동안엔 "선택된 연도"가 마치 결과에 적용된 것처럼 보이는 연도 필터를
  // 숨긴다. 유형별 현황 카드는 #normalView 안에 있어 필터 중엔 함께 숨겨지므로,
  // 결과 상단에 지금 뭐가 걸려있는지 + 전부 해제하는 버튼을 보여준다.
  document.getElementById('yearFilterGroup').hidden = filtering;
  ...
  if (filtering) {{
    ...
    if (activeCat !== 'all') {{
      const catBtn = document.querySelector(`[data-cat-btn][data-cat="${{activeCat}}"] .cat-card-label`);
      if (catBtn) chips.push(catBtn.textContent);
    }}
  ...
  document.querySelectorAll('#normalView .law-item').forEach(it => it.classList.remove('hidden'));
}}
