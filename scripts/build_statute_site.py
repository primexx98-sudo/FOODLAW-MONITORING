...
def render_articles(statute: dict) -> str:
    ...
    if not statute["text_available"]:
        ...
        # full_text는 카드엔 안 넣는다(AI 의미검색이 꺼져있는 동안엔 아예 안 쓰임) —
        # 건강기능식품/식품첨가물 공전 전문이 각각 수십만~2백만자라 그대로 넣으면 카드 하나가
        # 페이지 전체 용량을 배 이상 불려버림([[project_food_law_monitor]] 09-11 참고).
        # AI 의미검색은 2026-09-15 비활성화 유지(재활성화 시도했으나 사용자가 재차 보류
        # 요청) — 재활성화 시 ai_hint 블록도 함께 복원할 것.
        return f'''
...
<head>
  ...
  <style>
    *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
    ...
    .ai-result-item.ai-result-static{{cursor:default;}}
    .ai-result-item.ai-result-static:hover{{border-color:var(--hairline);}}
    .ai-result-score{{color:var(--muted);font-size:0.72rem;font-family:'JetBrains Mono',monospace;margin-left:6px;}}
    .ai-result-source{{color:var(--turquoise);font-size:0.7rem;font-weight:700;margin-top:3px;}}
...
<script>
  ...
  var practiceNames = {{}}; // kind="practice" 청크는 대응하는 DOM 카드가 없어 이 표로 이름 조회
  ...
  async function loadAiModel() {{
    ...
    try {{
      ...
      practiceNames = data.practice_names || {{}};

      var practiceCount = Object.keys(practiceNames).length;
      status.textContent = 'AI 의미검색 준비 완료 (' + aiChunks.length + '개 조문·별표' +
        (practiceCount ? ' + 실무자료 ' + practiceCount + '건' : '') + ' 인덱싱됨)';
  ...
  async function runAiSearch() {{
    ...
    // kind="practice"(실무자료)는 대응하는 <div class="statute-item"> 카드가 없어
    // (2026-09-11 사용자 로컬 파일에서 색인, 공개 사이트엔 원문을 안 싣기로 함)
    // DOM에서 이름을 못 찾으므로 practiceNames로 조회하고, 클릭해도 이동할 곳이
    // 없으니 버튼이 아닌 정적 항목(.ai-result-static)으로 렌더링 — 출처만 표시.
    results.innerHTML = '<div class="ai-results-label">🧠 의미 기반 추천</div>' + ranked.map(function(r) {{
      var isPractice = r.kind === 'practice';
      var name;
      if (isPractice) {{
        name = practiceNames[r.key] || r.key;
      }} else {{
        var nameEl = document.querySelector('#statute-' + CSS.escape(r.key) + ' .statute-name');
        name = nameEl ? nameEl.textContent : r.key;
      }}
      var sourceLine = isPractice ? '<div class="ai-result-source">출처: 실무자료</div>' : '';
      var tag = isPractice ? 'div' : 'button';
      var cls = isPractice ? 'ai-result-item ai-result-static' : 'ai-result-item';
      return '<' + tag + ' class="' + cls + '" data-key="' + r.key + '" data-kind="' + r.kind + '" data-no="' + r.no + '">' +
        name + '<span class="ai-result-score">유사도 ' + r.sim.toFixed(2) + '</span>' + sourceLine + '</' + tag + '>';
    }}).join('');
    results.hidden = false;
    results.querySelectorAll('.ai-result-item:not(.ai-result-static)').forEach(function(btn) {{
